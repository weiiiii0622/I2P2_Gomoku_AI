...
public:
    ...
    std::set<Point, SET_POINT_CMP> generatePossibleMoves(){
        std::set<Point, SET_POINT_CMP> possibleMoves;
        for(int i=0; i<SIZE; i++){
            for(int j=0; j<SIZE; j++){
    ...
    int evaluate_horizontal(bool my_turn){
        int score = 0;
        int dead = 2;
        int cont = 0;
        int target = -1;
        for(int i=0; i<SIZE; i++){
            for(int j=0; j<SIZE; j++){
                if(target==-1){
                    ...
                    if(cont > 0){
                        dead -= 1;
                        score += evaluate_cont_score(dead, cont, target==myColor);
                        cont = 0;
                        dead = 1;
                        target = -1;
                    }
    ...
    int evaluate_vertical(bool my_turn){
        int score = 0;
        int dead = 2;
        int cont = 0;
        int target = -1;
    ...
    int evaluate_diagonal(bool my_turn){
        int score = 0;
        int dead = 2;
        int cont = 0;
        int target = -1;
    ...
    int getScore(bool my_turn, bool init, int cmd){ //cmd 0:normal 1:searchWin 2:preventLose
        int score = 0;

        //score += (evaluate_horizontal(my_turn)+evaluate_vertical(my_turn)+evaluate_diagonal(my_turn));
        int my = (evaluate_horizontal(my_turn)+evaluate_vertical(my_turn)+evaluate_diagonal(my_turn));
        if(VALUE_FUNCTION && init) std::cout << "myscore: " << my << '\n';
        int op = (evaluate_horizontal(!my_turn)+evaluate_vertical(!my_turn)+evaluate_diagonal(!my_turn));
        if(VALUE_FUNCTION && init) std::cout << "opscore: " << op << '\n';
...
void write_valid_spot(std::ofstream& fout, State& state, int player) {
    //srand(time(NULL));
    int init_score = state.getScore(true, true, 0);
    if(DEBUG) std::cout << "Initial Score: " << init_score << '\n';

    std::set<Point, SET_POINT_CMP> possibleMoves = state.generatePossibleMoves();
    ...
    // if(player==1 && state.myCnt==1){
    //     if(state.board[SIZE/2-1][SIZE/2] == 0){
    //         fout << SIZE/2-1 << " " << SIZE/2 << '\n';
    //         fout.flush();
    //         return;
    //     }
    //     else if(state.board[SIZE/2+1][SIZE/2] == 0){
    //         fout << SIZE/2+1 << " " << SIZE/2 << '\n';
    //         fout.flush();
    //         return;
    //     }
    //     else if(state.board[SIZE/2][SIZE/2-1] == 0){
    //         fout << SIZE/2 << " " << SIZE/-1 << '\n';
    //         fout.flush();
    //         return;
    //     }
    //     else if(state.board[SIZE/2][SIZE/2+1] == 0){
    //         fout << SIZE/2 << " " << SIZE/2+1 << '\n';
    //         fout.flush();
    //         return;
    //     }
    //     else if(state.board[SIZE/2-1][SIZE/2-1] == 0){
    //         fout << SIZE/2-1 << " " << SIZE/2-1 << '\n';
    //         fout.flush();
    //         return;
    //     }
    //     else if(state.board[SIZE/2-1][SIZE/2+1] == 0){
...
Point getInitMove(State& state){
    ...
    for(auto move : possibleMoves){
        ...
        if(state.getScore(true, false, 1) >= WINNING_POINT){
            winningMove = move;
        ...
        int LS = state.getScore(false, false, 1);
...
int minimaxAB(State state, int depth, int alpha, int beta, bool maxPlayer, std::ofstream& fout){
    if(depth == 0){
        int finalScore = state.getScore(true, false, 0);
